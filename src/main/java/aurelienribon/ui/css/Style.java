 ...
 * objects, using CSS stylesheets. It is not directly linked with any technology
 * such as Swing, and therefore can be used with any kind of object, being UI
 * components, business models, or game elements.
 * <p/>
 * 
 * <h2>What is CSS?</h2>
 ...
 * declarations. Finally, a declaration is made of a property associated to a
 * value. The following image sums-up all of this:
 * <p/>
 * 
 ...
 * The engine is built around 3 kinds of components: {@link Property properties}, {@link Function functions}, and {@link DeclarationSetProcessor processors},
 * and you will need to register these components to the engine, since it comes
 * as naked as possible.
 * <p/>
 * 
 * <b>Important:</b> <font color="#AA0000">remember that the engine is
 * completely naked, this means that it comes without any registered property,
 * function, or processor. You first need to register these entities to the
 * engine to let it understand how to parse CSS stylesheets and apply them to
 * your targets. Some already made entities (for <b>Swing UIs</b> for instance)
 * can be found at the</font> <a
 * href="http://code.google.com/p/java-universal-css-engine/"> project
 * website</a>.
 * <p/>
 * 
...
public class Style
{
    ...
    private final static Map<String, Property>                           registeredProperties             = new LinkedHashMap<String, Property>();
    private final static Map<String, Function>                           registeredFunctions              = new LinkedHashMap<String, Function>();
    private final static Map<Class<?>, List<DeclarationSetProcessor<?>>> registeredProcessors             = new LinkedHashMap<Class<?>, List<DeclarationSetProcessor<?>>>();
    private final static Map<Object, List<String>>                       registeredTargets                = new LinkedHashMap<Object, List<String>>();
    private final static Map<Class<?>, ChildrenAccessor<?>>              registeredChildrenAccessors      = new HashMap<Class<?>, ChildrenAccessor<?>>();
    private final static Map<Class<?>, DeclarationSetManager<?>>         registeredDeclarationSetManagers = new HashMap<Class<?>, DeclarationSetManager<?>>();
    private final static Map<String, Class<?>>                           registeredTypes                  = new HashMap<String, Class<?>>();
     ...
     */
    public static void registerProcessor(Class<?> clazz,
            DeclarationSetProcessor<?> processor)
    {
     ...
     * "#something") to a target. Don't forget to unregister the target when you
     * dispose of it, to remove it from memory.
     */
    public static void registerCssClasses(Object target, String... classes)
    {
        if (!registeredTargets.containsKey(target))
        {
            registeredTargets.put(target, new ArrayList<String>());
        }
        for (final String name : classes)
        {
            if (name.startsWith("#"))
            {
                for (final List<String> names : registeredTargets.values())
                {
            ...
            else
            {
                throw new RuntimeException(
                        "Names have to start with a '.' for classes or a '#' for ids");
            }
        }
    }
     ...
     */
    public static void registerChildrenAccessor(Class<?> parentClass,
     ...
     */
    public static void registerDeclarationSetManager(Class<?> parentClass,
            DeclarationSetManager<?> manager)
    {
    ...
    {
        for (final Class<?> clazz : registeredProcessors.keySet())
        {
            if (clazz.isInstance(target))
            {
                final List<DeclarationSetProcessor<?>> processors = registeredProcessors
                        .get(clazz);
                for (final DeclarationSetProcessor<?> proc : processors)
                {
                    ((DeclarationSetProcessor<Object>) proc)
                            .process(target, ds);
                }
            }
        }
    ...
    {
        final List<String> classes = registeredTargets.get(target);
        if (classes != null)
        {
            return classes;
        }
    ...
    public static Map<Class<?>, ChildrenAccessor<?>> getRegisteredChildrenAccessors()
    {
        return registeredChildrenAccessors;
    }
    ...
    {
        styleSheet = getStyleSheet(styleSheetUrl);
        parse(styleSheet);
    }
    ...
    {
        final CharStream cs = new ANTLRStringStream(styleSheet);
        final CssLexer lexer = new CssLexer(cs);
        ...
        for (final CssParser.Rule parserRule : parser.rules)
        {
            final List<Property> properties = new ArrayList<Property>();
            final Map<Property, List<Object>> propertiesValues = new HashMap<Property, List<Object>>();
            
            for (final String name : parserRule.declarations.keySet())
            {
                final Property property = registeredProperties.get(name);
                if (property == null)
                {
                    throw StyleException.forProperty(name);
                }
                
                final List<Object> params = parserRule.declarations.get(name);
            ...
            for (final CssParser.Selector rawSelector : parserRule.selectors)
            {
                final Selector selector = new Selector(rawSelector.str);
                final DeclarationSet ds = new DeclarationSet(this, properties,
                        propertiesValues);
    ...
    private boolean checkParams(Parameterized call, List<Object> params)
    {
        ...
        for (final Class<?>[] classes : call.getParams())
        {
    ...
    private boolean isMatch(List<Object> params, Class<?>[] classes)
    {
        ...
        for (int i = 0; i < params.size(); i++)
        {
            ...
            final Class<?> paramClass = getParamClass(param);
    ...
    private Class<?> getParamClass(Object param)
    {
        ...
        {
            final CssParser.Function parserFunction = (CssParser.Function) param;
            final Function function = registeredFunctions
                    .get(parserFunction.name);
            if (function == null)
            {
                throw StyleException.forFunction(parserFunction.name);
            }
            
            final List<Object> functionParams = parserFunction.params;
            if (!checkParams(function, functionParams))
            {
    ...
    @SuppressWarnings("unchecked")
    private static void applyImpl(Object target, Style style,
            List<Selector.Atom> stack)
    {
        // Retrieve all the declarations belonging to the target and evaluate
        // their parameters
        final Map<String, DeclarationSet> dss = DeclarationSet.dss(style,
                target, stack);
        
        for (final DeclarationSet ds : dss.values())
        {
            for (final Property property : ds.getProperties())
            {
                for (int i = 0; i < ds.getValue(property).size(); i++)
                {
                    final Object param = ds.getValue(property).get(i);
        ...
        final Selector.Atom atom = new Selector.Atom(target.getClass(),
                getRegisteredTargetClassNames(target));
        ...
        for (final Class<?> clazz : registeredDeclarationSetManagers.keySet())
        {
            if (clazz.isInstance(target))
            {
                final DeclarationSetManager<Object> manager = (DeclarationSetManager<Object>) registeredDeclarationSetManagers
                        .get(clazz);
        ...
        {
            final Container parent = (Container) target;
            if (parent.getChildren() != null)
            {
                for (final Object child : parent.getChildren())
                {
                    apply(child, style, stack);
                }
            }
        ...
        {
            for (final Class<?> clazz : registeredChildrenAccessors.keySet())
            {
                if (clazz.isInstance(target))
                {
                    final ChildrenAccessor<Object> accessor = (ChildrenAccessor<Object>) registeredChildrenAccessors
                            .get(clazz);
                    if (accessor.getChildren(target) != null)
                    {
                        for (final Object child : accessor.getChildren(target))
                        {
                            apply(child, style, stack);
                        }
                    }
    ...
    private static Object evaluateParam(Object param) throws StyleException
    {
        if (param instanceof CssParser.Color)
        {
            final CssParser.Color c = (CssParser.Color) param;
            if (converter != null)
            {
                return converter.convertColor(parseColor(c.str));
            }
        }
    ...
    private static String generatePropertiesManual()
    {
        ...
        for (final Property property : registeredProperties.values())
        {
            ...
            for (int i = 0; i < property.getParams().length; i++)
            {
                ...
                {
                    Class<?>[][] paramsClasses = property.getParams();
                    String[][] paramsNames = property.getParamsNames();
                    
                    final String clazz = paramsClasses[i][ii].getSimpleName();
                    final String name = paramsNames[i][ii];
    ...
    private static String generateFunctionsManual()
    {
        ...
        for (final Function function : registeredFunctions.values())
        {
            ...
            for (int i = 0; i < function.getParams().length; i++)
            {
                ...
                {
                    final String clazz = function.getParams()[i][ii]
                            .getSimpleName();
                    final String name = function.getParamsNames()[i][ii];
    ...
    {
        final int incr = s.length() == 9 || s.length() == 7 ? 2 : 1;
        final boolean alpha = s.length() == 9 || s.length() == 5;
        
        int i = 1;
        final int a = alpha ? hex(s.substring(i, i += incr)) : 255;
        final int r = hex(s.substring(i, i += incr));
        final int g = hex(s.substring(i, i += incr));
        final int b = hex(s.substring(i, i += incr));
