...
public class PixmapBorder extends Border
{
    ...
    public PixmapBorder(InputOnlyStream stream) throws IOException,
    ...
    /**
     * Creates a new PixmapBorder. Fill in the pixmaps counter clock-wise which
     * is FengGUI convention.
     * 
     * @param array
     *            Contains the pixmaps for this border in the proper order. The
     *            order of the pixmaps in the array is given in the class
     *            description of PixmapBorder. Generally we suggest to directly
     *            use the static fields TOP_LEFT, TOP, TOP_RIGHT, etc. to fill
     *            the array, because we use these static fields internally as
     *            well.
    ...
    public PixmapBorder(Pixmap[] array) throws IllegalArgumentException
    {
        ...
        if (array.length == 8)
        {
            setSpacing(array[TOP].getHeight(), array[LEFT].getWidth(),
                    array[RIGHT].getWidth(), array[BOTTOM].getHeight());
        }
        else
        {
            throw new IllegalArgumentException(
                    "PixmapBorder takes 8 pixmaps, not " + array.length);
        }
     ...
     */
    public PixmapBorder(Pixmap leftEdge, Pixmap rightEdge, Pixmap topEdge,
            Pixmap bottomEdge, Pixmap upperLeftCorner, Pixmap upperRightCorner,
            Pixmap lowerLeftCorner, Pixmap lowerRightCorner)
    {
        this(new Pixmap[] { upperLeftCorner, leftEdge, lowerLeftCorner,
                bottomEdge, lowerRightCorner, rightEdge, upperRightCorner,
                topEdge });
    }
    ...
    public void paint(Graphics g, int localX, int localY, int width, int height)
    {
        ...
        g.drawScaledImage(tex[LEFT], localX, localY + getBottom(), getLeft(),
                height - getBottom() - getTop());
        
        g.drawScaledImage(tex[BOTTOM], localX + tex[BOTTOM_LEFT].getWidth(),
                localY, width - getLeft() - getRight(), getBottom());
        
        g.drawScaledImage(tex[TOP], localX + getLeft(), localY + height
                - getTop(), width - getLeft() - getRight(), getTop());
        
        g.drawScaledImage(tex[RIGHT], localX + width - getRight(), localY
                + getBottom(), getRight(), height - getBottom() - getTop());
            ...
            IXMLStreamableException
    {
        ...
        tex[TOP_LEFT] = stream.processChild("TopLeftPixmap", tex[TOP_LEFT],
                Pixmap.class);
        tex[LEFT] = stream.processChild("LeftEdgePixmap", tex[LEFT],
                Pixmap.class);
        tex[BOTTOM_LEFT] = stream.processChild("BottomLeftPixmap",
                tex[BOTTOM_LEFT], Pixmap.class);
        tex[BOTTOM] = stream.processChild("BottomEdgePixmap", tex[BOTTOM],
                Pixmap.class);
        tex[BOTTOM_RIGHT] = stream.processChild("BottomRightPixmap",
                tex[BOTTOM_RIGHT], Pixmap.class);
        tex[RIGHT] = stream.processChild("RightEdgePixmap", tex[RIGHT],
                Pixmap.class);
        ...
        super.setSpacing(tex[TOP].getHeight(), tex[LEFT].getWidth(),
                tex[RIGHT].getWidth(), tex[BOTTOM].getHeight());
    }
