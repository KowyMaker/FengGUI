...
public class SetMarginSwitch extends Switch
{
     ...
     *            label of switch.
     * @param defaultSpace
     *            The default spacing to use for Margin.
     * @param upDown
     *            the value to change the margin on top and bottom.
     * @param leftRight
     *            the value to change the margin on left and right.
     */
    public SetMarginSwitch(String label, Spacing defaultSpace, int upDown,
            int leftRight)
    {
        super(label);
        this.upDown = upDown;
    ...
    public void setup(IWidget widget)
    {
        ...
        {
            final StandardWidget stdWid = (StandardWidget) widget;
            
            final IAppearance appearance = stdWid.getAppearance();
            if (appearance instanceof SpacingAppearance)
            {
                final SpacingAppearance spaceApp = (SpacingAppearance) appearance;
                spaceApp.setMargin(new Spacing(
                        defaultSpacing.getTop() + upDown, defaultSpacing
                                .getLeft() + leftRight, defaultSpacing
                                .getRight() - leftRight, defaultSpacing
                                .getBottom() - upDown));
            }
        }
    }
