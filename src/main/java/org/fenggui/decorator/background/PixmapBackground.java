...
/**
 * Draws pixmaps as the background of a widget. The
 * <coder>PixmapBackground</code> has two modes of operation:
 * <ul>
 * <li>A single pixmap that is drawn in the center or scaled to fill the widget.
 * It can be used to display a plain image as the background.</li>
 * <li>A single pixmap that is drawn in the center and eight border-pixmaps that
 * surround the pixmap in the center. Again, the pixmap in the center can be
 * scaled to fill the residual space. This mode can be used to draw sharp edges
 * where the inner pixmap is scaled. This comes in handy for inlay-borders for
 * example.</li>
 * </ul>
 * The pixmaps are aligned as follows where the centered pixmap can fill the
 * entire space when no surrounding pixmaps are specified:
 * 
 * <pre>
 * +-------------+--------+--------------+
...
public class PixmapBackground extends Background
{
     ...
     * 
     * @param center
     *            the Texture to draw as the background
     */
    ...
    public PixmapBackground(InputOnlyStream stream) throws IOException,
     ...
     *            the Texture to draw as the background
     * @param scaled
     *            if the texture need to be scaled
     */
     ...
     * 
     * @param center
     *            the pixmap in the center
     * @param topLeft
     *            the pixmap in the upper left corner
     * @param top
     *            the pixmap at the top
     * @param topRight
     *            the pixmap at the upper right corner
     * @param right
     *            the pxiamp at the right
     * @param bottomRight
     *            the pixmap at the lower right corner
     * @param bottom
     *            the pixmap at the bottom
     * @param bottomLeft
     *            the pixmap at the lower left corner
     * @param left
     *            the pixmap to the left
     * @param scaled
     *            flag wether the pixmap in the center shall be scaled.
     */
    public PixmapBackground(Pixmap center, Pixmap topLeft, Pixmap top,
            Pixmap topRight, Pixmap right, Pixmap bottomRight, Pixmap bottom,
            Pixmap bottomLeft, Pixmap left, boolean scaled)
    {
        this(center, topLeft, top, topRight, right, bottomRight, bottom,
                bottomLeft, left, scaled, false);
    }
     ...
     * @param scaled
     *            flag wether the pixmap in the center shall be scaled.
     * @param useAlternateBlending
     *            use alternate blending mode for this decorator.
     */
    public PixmapBackground(Pixmap center, Pixmap topLeft, Pixmap top,
            Pixmap topRight, Pixmap right, Pixmap bottomRight, Pixmap bottom,
            Pixmap bottomLeft, Pixmap left, boolean scaled,
    ...
    private void checkIntegrity()
    {
        if (bottomLeft.getHeight() != bottom.getHeight())
        {
            throw new IllegalArgumentException(
                    "bottomLeft.getHeight() != bottom.getHeight()");
        }
        
        if (bottom.getHeight() != bottomRight.getHeight())
        {
            throw new IllegalArgumentException(
                    "bottom.getHeight() != bottomRight.getHeight()");
        }
        
        if (bottomLeft.getWidth() != left.getWidth())
        {
            throw new IllegalArgumentException(
                    "bottomLeft.getWidth() != left.getWidth()");
        }
        
        if (left.getWidth() != topLeft.getWidth())
        {
            throw new IllegalArgumentException(
                    "left.getWidth() != topLeft.getWidth()");
        }
        
        if (topLeft.getHeight() != top.getHeight())
        {
            throw new IllegalArgumentException(
                    "topLeft.getHeight() != top.getHeight()");
        }
        
        if (top.getHeight() != topRight.getHeight())
        {
            throw new IllegalArgumentException(
                    "top.getHeight() != topRight.getHeight()");
        }
        
        if (topRight.getWidth() != right.getWidth())
        {
            throw new IllegalArgumentException(
                    "topRight.getWidth() != right.getWidth()");
        }
        
        if (right.getWidth() != bottomRight.getWidth())
        {
            throw new IllegalArgumentException(
                    "right.getWidth() != bottomRight.getWidth()");
        }
    }
    ...
    public void paint(Graphics g, int localX, int localY, int width, int height)
    {
        if (useAlternateBlending)
        {
            ...
            g.setColor(new Color(blendingColor.getRed()
                    * blendingColor.getAlpha(), blendingColor.getGreen()
                    * blendingColor.getAlpha(), blendingColor.getBlue()
                    * blendingColor.getAlpha(), blendingColor.getAlpha()));
        }
        else
        {
        ...
        if (topLeft == null)
        {
            ...
            {
                g.drawImage(center, localX + width / 2 - center.getWidth() / 2,
                        localY + height / 2 - center.getHeight() / 2);
            }
        }
        else
        // draw the "border" around the center
        {
            ...
            g.drawScaledImage(bottom, localX + bottomLeft.getWidth(), localY,
                    width - left.getWidth() - right.getWidth(),
                    bottom.getHeight());
            g.drawImage(bottomRight, localX + width - bottomRight.getWidth(),
                    localY);
            
            g.drawScaledImage(left, localX, localY + bottomLeft.getHeight(),
                    left.getWidth(),
                    height - top.getHeight() - bottom.getHeight());
            
            g.drawScaledImage(right, localX + width - right.getWidth(), localY
                    + bottomRight.getHeight(), right.getWidth(), height
                    - topRight.getHeight() - bottomRight.getHeight());
            
            g.drawImage(topLeft, localX, localY + height - top.getHeight());
            g.drawScaledImage(top, localX + topLeft.getWidth(), localY + height
                    - top.getHeight(),
                    width - topRight.getWidth() - topLeft.getWidth(),
                    topLeft.getHeight());
            g.drawImage(topRight, localX + width - topRight.getWidth(), localY
                    + height - topRight.getHeight());
            
            if (scaled)
            {
                g.drawScaledImage(center, localX + left.getWidth(), localY
                        + bottomLeft.getHeight(), width - right.getWidth()
                        - left.getWidth(),
                        height - top.getHeight() - bottom.getHeight());
            }
            else
            ...
            IXMLStreamableException
    {
        ...
        blendingColor = stream.processChild("BlendingColor", blendingColor,
                DEFAULT_BLENDING_COLOR, Color.class);
        ...
        topLeft = stream.processChild("TopLeftPixmap", topLeft, null,
                Pixmap.class);
        topRight = stream.processChild("TopRightPixmap", topRight, null,
                Pixmap.class);
        bottom = stream
                .processChild("BottomPixmap", bottom, null, Pixmap.class);
        bottomLeft = stream.processChild("BottomLeftPixmap", bottomLeft, null,
                Pixmap.class);
        bottomRight = stream.processChild("BottomRightPixmap", bottomRight,
                null, Pixmap.class);
    }
