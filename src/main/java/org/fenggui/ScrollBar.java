...
/**
 * Implementation of a scroll bar widget. It consists of two buttons and a
 * slider.
 * 
 ...
 *         $Date: 2007-11-19 14:53:14 +0100 (Mo, 19 Nov 2007) $
 * @version $Revision: 617 $
 */
public class ScrollBar extends StandardWidget implements IBasicContainer,
        Cloneable
{
    ...
    public ScrollBar(InputOnlyStream stream) throws IOException,
     ...
     * 
     * @param horizontal
     *            true if the Slider of the ScrollBar shall be moved
    ...
    public void layout()
    {
        ...
        if (horizontal)
        {
            ...
            slider.setSize(
                    contentWidth
                            - (increaseBtn.getWidth() + decreaseBtn.getWidth()),
                    contentHeight);
        }
        else
        {
            ...
            slider.setSize(
                    contentWidth,
                    contentHeight
                            - (decreaseBtn.getHeight() + increaseBtn
                                    .getHeight()));
        }
        slider.layout();
    }
    ...
    public IWidget getWidget(int x, int y)
    {
        ...
        if (decreaseBtn.getSize().contains(x - decreaseBtn.getX(),
                y - decreaseBtn.getY()))
        {
            return decreaseBtn;
        }
        if (increaseBtn.getSize().contains(x - increaseBtn.getX(),
                y - increaseBtn.getY()))
        {
            return increaseBtn;
        }
    ...
    public Dimension getMinContentSize()
    {
        ...
        if (isHorizontal())
        {
            final int height = Math.max(getIncreaseButton().getMinHeight(),
                    Math.max(getDecreaseButton().getMinHeight(), getSlider()
                            .getMinHeight()));
            return new Dimension(getIncreaseButton().getMinWidth()
                    + getDecreaseButton().getMinWidth()
                    + getSlider().getMinWidth() + 1, height);
        }
        else
        {
            return new Dimension(Math.max(getIncreaseButton().getMinWidth(),
                    Math.max(getDecreaseButton().getMinWidth(), getSlider()
                            .getMinWidth())), getIncreaseButton()
                    .getMinHeight()
    ...
    public ScrollBar clone()
    {
        ...
        result.slider = slider.clone();
        result.increaseBtn = increaseBtn.clone();
        result.decreaseBtn = decreaseBtn.clone();
