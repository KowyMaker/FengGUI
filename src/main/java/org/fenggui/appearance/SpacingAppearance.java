 ...
 * 
 * * The following ASCII-Art is supposed to illustrate the margins.
 * 
 * <pre>
 * +---------------------------------------------------------------------------------------------------+
 * |                                                |                                                  |
...
public abstract class SpacingAppearance implements IAppearance, IXMLStreamable
{
    ...
    public SpacingAppearance(StandardWidget w, SpacingAppearance appearance)
    {
        this(w);
        widget = w;
        margin = new Spacing(appearance.margin);
        border = new Spacing(appearance.border);
        padding = new Spacing(appearance.padding);
    }
    
    public SpacingAppearance(StandardWidget w, InputOnlyStream stream)
     ...
     * 
     * @param border
     *            the spacing which says how much space to reserve
     */
    ...
    public final Dimension getMinSizeHint()
    {
        ...
        contentSize.setSize(
                contentSize.getWidth() + border.getLeftPlusRight()
                        + margin.getLeftPlusRight()
                        + padding.getLeftPlusRight(),
                contentSize.getHeight() + border.getBottomPlusTop()
    ...
    /**
     * Translates the coordiante system to the lower left corner of the content
     * area. It calls <code>paintBackgroundContent</code> to actually draw the
     * content.
     */
    ...
    /**
     * Translates the coordiante system to the lower left corner of the content
     * area. It calls <code>paintForeground</code> to actually draw the content.
     */
    ...
    public void paint(Graphics g, IOpenGL gl)
    {
        final int offsetX = margin.getLeft() + border.getLeft()
                + padding.getLeft();
        final int offsetY = margin.getBottom() + border.getBottom()
                + padding.getBottom();
    ...
    {
        return widget.getSize().getWidth() - border.getLeftPlusRight()
                - margin.getLeftPlusRight() - padding.getLeftPlusRight();
    }
    ...
    {
        return widget.getSize().getHeight() - border.getBottomPlusTop()
                - margin.getBottomPlusTop() - padding.getBottomPlusTop();
    }
    
    /**
     * Returns if the specified point lays inside the box without margin.
     * 
     * @param pX
    ...
    public final boolean insideMargin(int pX, int pY)
    {
        ...
        final int innerWidth = getContentWidth();
        final int innerHeight = getContentHeight();
        
        return pX >= 0
                && pX < innerWidth + padding.getLeft() + padding.getRight()
                        + getBorder().getLeft() + getBorder().getRight()
                && pY >= 0
                && pY < innerHeight + padding.getBottom() + padding.getTop()
                        + getBorder().getTop() + getBorder().getBottom();
    }
    
    /**
     * Returns the sum of bottom margin, bottom border and the bottom padding.
     * 
     * @return sum
     */
    public int getBottomMargins()
    {
        return margin.getBottom() + getBorder().getBottom()
                + padding.getBottom();
    }
    ...
    /**
     * Returns the sum of the left margin, left border and left padding.
     * 
     * @return sum
     */
    ...
    /**
     * Returns the sum of the right margin, right border and right padding.
     * 
     * @return sum
     */
    ...
    {
        return getWidget().getMinSize().getWidth() - getLeftMargins()
                - getRightMargins();
    }
    
    public int getContentMinHeight()
    {
        return getWidget().getMinSize().getHeight() - getTopMargins()
                - getBottomMargins();
    }
    ...
    {
        String s = "";
        s += "content size         : " + getContentWidth() + ", "
                + getContentHeight() + "\n";
    ...
    {
        border = stream.processChild("Border", border, Spacing.ZERO_SPACING,
                Spacing.class);
        margin = stream.processChild("Margin", margin, Spacing.ZERO_SPACING,
                Spacing.class);
    ...
    protected SpacingAppearance clone(StandardWidget widget)
    {
        ...
        result.margin = margin.clone();
        result.padding = padding.clone();
