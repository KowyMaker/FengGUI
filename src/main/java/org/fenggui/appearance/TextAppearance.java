...
public class TextAppearance extends DefaultAppearance
{
    ...
    public TextAppearance(StandardWidget w, TextAppearance appearance)
    {
        ...
        alignment = appearance.getAlignment();
        
        setDefaultRenderer();
    }
    
    public TextAppearance(StandardWidget w, InputOnlyStream stream)
    ...
    private void setDefaultRenderer()
    {
        ...
        {
            renderers.put(ITextRenderer.DEFAULTTEXTRENDERERKEY,
                    ITextRenderer.DEFAULTRENDERER);
        }
            ...
            IXMLStreamableException
    {
        ...
        alignment = stream.processEnum("alignment", alignment,
                Alignment.MIDDLE, Alignment.class, Alignment.STORAGE_FORMAT);
    ...
    public ITextRenderer getRenderer(String name)
    {
        ...
        if (ITextRenderer.DEFAULTTEXTRENDERERKEY.equals(name))
        {
            throw new MissingResourceException(
                    "Default TextRenderer not defined in appearance.",
                    "ITextRenderer", ITextRenderer.DEFAULTTEXTRENDERERKEY);
        }
        else
        {
    ...
    public TextStyle getStyle(String key)
    {
        ...
        {
            throw new MissingResourceException(
                    "Default Style not defined in appearance", "TextStyle",
    ...
    public TextAppearance clone(StandardWidget widget)
    {
        ...
        result.renderers = new HashMap<String, ITextRenderer>(renderers.size());
        result.styles = new HashMap<String, TextStyle>(styles.size());
        
        // Renderers should be fixed objects, for new renderer a new object
        // needs to be created
        result.renderers.putAll(renderers);
        
        for (final Entry<String, TextStyle> style : styles.entrySet())
        {
