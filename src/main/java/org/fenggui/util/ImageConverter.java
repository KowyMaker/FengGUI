...
public class ImageConverter
{
    ...
    public final static ComponentColorModel COLOR_MODEL = new ComponentColorModel(
                                                                ColorSpace
    ...
    public static BufferedImage createGlCompatibleAwtImage(int width, int height)
    {
        ...
        final BufferedImage convertImage = new BufferedImage(COLOR_MODEL,
    ...
    {
        final int nearestPower = powerOf2(value);
    ...
    public static int powerOf2(int value)
    {
        if (value < 3)
        {
            throw new IllegalArgumentException(
                    "value must be greater or equals 3!");
        }
        
        final int power = Integer.highestOneBit(value - 1);
        return 2 * power;
    }
    
    /**
     * Converts the given image to a gl compatible format if necessary and
     * returns the data in the format GL_RGBA as GL_UNSIGNED_BYTE.
     * 
     * @param awtImage
    ...
    public static ByteBuffer convert(BufferedImage awtImage)
    {
        ...
        {
            final BufferedImage convertImage = createGlCompatibleAwtImage(
                    awtImage.getWidth(), awtImage.getHeight());
            // copy the source image into the produced image
            final Graphics g = convertImage.getGraphics();
        ...
        final byte[] data = ((DataBufferByte) awtImage.getRaster()
    ...
    /**
     * Converts the given image to a gl compatible format if necessary and
     * returns the data in the format GL_RGBA as GL_UNSIGNED_BYTE such that the
     * width and height are a power of 2. The image is place in the upper left
     * corner.
     * 
     * @param awtImage
     *            the image to be converted to an byte buffer
    ...
    {
        if (!isGlCompatibleAwtImage(awtImage)
                || !isPowerOf2(awtImage.getWidth())
                || !isPowerOf2(awtImage.getHeight()))
        {
            final int width = powerOf2(awtImage.getWidth());
            final int height = powerOf2(awtImage.getHeight());
            final BufferedImage convertImage = createGlCompatibleAwtImage(
                    width, height);
            // copy the source image into the produced image
            final Graphics g = convertImage.getGraphics();
