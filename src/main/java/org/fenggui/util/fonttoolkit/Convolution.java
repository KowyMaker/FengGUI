...
public class Convolution extends RenderStage
{
    ...
    {
        final double[][] red = new double[image.getWidth()][image.getHeight()];
        final double[][] green = new double[image.getWidth()][image.getHeight()];
        final double[][] blue = new double[image.getWidth()][image.getHeight()];
        final double[][] alpha = new double[image.getWidth()][image.getHeight()];
        
        final ColorModel cm = image.getColorModel();
        
        for (int x = kernel.getWidth() / 2; x < image.getWidth()
                - kernel.getWidth() / 2; x++)
        {
            for (int y = kernel.getHeight() / 2; y < image.getHeight()
                    - kernel.getHeight() / 2; y++)
            {
                ...
                for (int j = 0; j < kernel.getWidth(); j++)
                {
                    for (int k = 0; k < kernel.getHeight(); k++)
                    {
                        ...
                        final int rgb = image.getRGB(x + j - kernel.getWidth()
                                / 2, y + k - kernel.getHeight() / 2);
                        
                        final double value = kernel.getValue(j, k);
        ...
        for (int x = 0; x < image.getWidth(); x++)
        {
            for (int y = 0; y < image.getHeight(); y++)
            {
                ...
                final Color color = new Color((int) red[x][y],
                        (int) green[x][y], (int) blue[x][y], (int) alpha[x][y]);
                g.setColor(color);
                g.drawLine(x, y, x, y);
            }
        }
    ...
    private void normalize(double[][] red, double[][] green, double[][] blue,
            double[][] alpha)
    {
