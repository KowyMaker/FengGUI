...
public class DrawCharacterOutline extends RenderStage
{
            ...
            char c, int safetyMargin)
    {
        ...
        final GlyphVector v = fontMetrics.getFont().createGlyphVector(
                g.getFontRenderContext(), new char[] { c });
        final Shape shape = v.getOutline(safetyMargin / 2,
                fontMetrics.getHeight() - safetyMargin / 2);
        g.setColor(color);
        g.draw(shape);
    }
