 ...
 *         2009-03-25 23:02:20 +0100 (Mi, 25 Mär 2009) $
...
public class Rectangle implements Cloneable
{
     ...
     * 
     * @param rect
     * @return <code>true</code> if <code>rect</code> instersects this
     *         rectangle.
     */
    ...
    {
        final int x = Math.max(rect.getX(), this.getX());
        final int y = Math.max(rect.getY(), this.getY());
        
        final int w = Math.min(rect.getX() + rect.getWidth(), this.getX()
                + this.getWidth())
                - Math.max(rect.getX(), this.getX());
        
        final int h = Math.min(rect.getY() + rect.getHeight(), this.getY()
                + this.getHeight())
