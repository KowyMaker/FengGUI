...
public class Item implements IXMLStreamable, IToggable<Item>
{
    ...
    public Item(InputOnlyStream stream) throws IOException,
    ...
    public void render(int x, int y, int width, int height, Item hoveredItem,
    ...
    protected void renderContent(int x, int y, int width, int height,
            Item hoveredItem, EntryAppearance appearance, Graphics g)
    {
        int itemWidth = 0;
        int itemHeight = 0;
        ...
        if (pixmap != null)
        {
            ...
            final int localY = y
                    + appearance.getAlignment().alignY(itemHeight,
                            pixmap.getHeight());
