 ...
 * easily render images as well as text.<br/>
 * <br/>
 * Note that toggle buttons are not supported in the current implementation of
 * Button.<br/>
 * <br/>
 * 
 * 
 * A Button has all states from a StatefullWidget and the additional state
 * pressed.
 * 
 * 
 * @author Johannes Schaback, last edited by $Author: marcmenghin $, $Date:
 *         2008-03-12 12:25:07 +0100 (Mi, 12 Mrz 2008) $
...
public class Button extends ObservableLabelWidget
{
    ...
    public Button(InputOnlyStream stream) throws IOException,
    ...
    public void keyPressed(KeyPressedEvent keyPressedEvent)
    {
        if (isEnabled())
        {
            ...
            {
                if (keyPressedEvent.getKey() == ' '
                        || keyPressedEvent.getKeyClass() == Key.ENTER)
                {
                    pressed();
    ...
    public void mousePressed(MousePressedEvent mousePressedEvent)
    {
        if (isEnabled())
        {
            if (!mousePressedEvent.isAlreadyUsed()
                    && mousePressedEvent.getButton() == MouseButton.LEFT)
            {
                pressed();
    ...
    public void mouseReleased(MouseReleasedEvent mouseReleasedEvent)
    {
        if (isEnabled())
        {
            if (!mouseReleasedEvent.isAlreadyUsed()
                    && mouseReleasedEvent.getButton() == MouseButton.LEFT)
            {
    ...
    public void mouseEntered(MouseEnteredEvent mouseEnteredEvent)
    {
        if (isEnabled())
        {
            if (!pressed)
            {
                if (mouseEnteredEvent.getButton() == MouseButton.LEFT
                        && mouseEnteredEvent.getClickCount() >= 1)
                {
                    pressed();
    ...
    {
        final ButtonPressedEvent e = new ButtonPressedEvent(this, modifiers);
        
        for (int i = 0; i < buttonPressedHook.size(); i++)
        {
            final IButtonPressedListener l = buttonPressedHook.get(i);
            l.buttonPressed(e);
        }
    }
