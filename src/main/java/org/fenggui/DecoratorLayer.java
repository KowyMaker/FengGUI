...
/**
 * Class to make decorators usable for overlay effects such as selections bars
 * in menus und tables.
 * 
...
public class DecoratorLayer implements IXMLStreamable, Cloneable
{
    ...
    public DecoratorLayer(InputOnlyStream stream) throws IOException,
    ...
    {
        for (final IDecorator deco : layer.decorators)
        {
            decorators.add(deco);
        }
    }
    ...
    {
        for (final IDecorator deco : decorators)
        {
            if (!deco.isEnabled())
            {
                continue;
