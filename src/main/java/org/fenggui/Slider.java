...
/**
 * Implementation of a slider Widget. Sliders are used to quickly (and roughly)
 * select numbers in a certain range. For example to select a percentage between
 * 0% and 100%. Of course other values are possible as well but the implementor
 * has to map the numbers into the desired domain. By moving the Slider the user
 * modifies the slider <code>value</code> between 0 and 1. This value can be
 * queried through the <code>getValue()</code> method.
 * 
 * @author Johannes Schaback
 * @dedication The Beatles - Sgt. Peppers Lonely Hearts Club Band
 */
public class Slider extends StatefullWidget<DefaultAppearance> implements
        IBasicContainer, Cloneable
{
    private ArrayList<ISliderMovedListener> sliderMovedHook = new ArrayList<ISliderMovedListener>();
    
    /**
     * SliderMovedEvent event that is used when the according event occurs. I
     * buffer it here so that we do not need to re-instantiate every time the
     * slider was dragged.
     */
    private SliderMovedEvent                sliderMoved     = new SliderMovedEvent(
                                                                    this);
     ...
     * 
     * @param horizontal
     *            flag that indicates whether the Slider is moved vertically or
     *            horizontally.
     */
    ...
    public Slider(InputOnlyStream stream) throws IOException,
    ...
    /**
     * Returns the selected value according to the position of the slider in a
     * range between 0 and 1. For horizontal Sliders, the 0 value is at the left
     * most side. For vertical Sliders, the 0 value is at the bottom.
     * 
    ...
    /**
     * Sets the value of the Slider. The value stands for his position. The
     * value has to be between 0 and 1. Values outside of this domain get
     * cropped. Emits a SliderMove signal.
     * 
     * @param position
    ...
    public void setValue(double position)
    {
        ...
        {
            sliderButton
                    .setX((int) ((getAppearance().getContentWidth() - sliderButton
                            .getWidth()) * value));
        }
        else
        {
            sliderButton
                    .setY((int) ((getAppearance().getContentHeight() - sliderButton
                            .getHeight()) * value));
        }
        
        // System.out.println(horizontal+" "+value+" "+sliderButton+" "+(double)(getInnerSize().getWidth()
    ...
    public IWidget getWidget(int x, int y)
    {
        ...
        if (sliderButton.getSize().contains(x - sliderButton.getX(),
                y - sliderButton.getY()))
        {
            return sliderButton;
        }
    ...
    private class SliderDnDListener implements IDragAndDropListener
    {
        ...
        private final Slider sliderButton;
        ...
        public void select(int x, int y, Set<Key> modifiers)
        {
            ...
            cacheDisplayY = sliderButton.getDisplayY();
            x -= cacheDisplayX;
            y -= cacheDisplayY;
        ...
        public void drag(int x, int y, Set<Key> modifiers)
        {
            if (horizontal)
            {
                ...
                sliderButton
                        .setValue((double) x
                                / (double) (sliderButton.getWidth() - sliderButton.sliderButton
                                        .getWidth()));
                
                sliderButton.fireSliderMovedEvent();
            ...
            else
            {
                ...
                sliderButton
                        .setValue((double) y
                                / (double) (sliderButton.getHeight() - sliderButton.sliderButton
                                        .getHeight()));
        ...
        {
            return w.equals(sliderButton.sliderButton);
        }
    }
    ...
    /**
     * Returns the start position of the Slider Button within the Slider Widget.
     * This method a x-coordinate value if the Slider is set to horizontal, a
     * y-coordinate value otherwise.
     * 
    ...
    public int getSliderStart()
    {
        if (horizontal)
        {
            return (int) (value * (getWidth() - sliderButton.getWidth()));
        }
        else
        {
            return (int) (value * (getHeight() - sliderButton.getHeight()));
        }
    }
    ...
    public void layout()
    {
        ...
        {
            int width = Math.max(
                    (int) (getAppearance().getContentWidth() * buttonSize),
                    sliderButton.getMinWidth());
            if (width < 15)
            {
                width = 15;
            }
        ...
        {
            int height = Math.max(
                    (int) (getAppearance().getContentHeight() * buttonSize),
                    sliderButton.getMinHeight());
            if (height < 15)
            {
                height = 15;
            }
    ...
    public void mousePressed(MousePressedEvent mousePressedEvent)
    {
        ...
        if (isEnabled())
        {
            if (isHorizontal())
            {
                final int x = mousePressedEvent.getDisplayX() - getDisplayX();
                final double size = (double) sliderButton.getWidth()
                        / (double) getAppearance().getContentWidth();
                if (x < getSliderStart())
                {
            ...
            {
                final int y = mousePressedEvent.getDisplayY() - getDisplayY();
                final double size = (double) sliderButton.getHeight()
                        / (double) getAppearance().getContentHeight();
    ...
    public void setSize(double d)
    {
        ...
        if (horizontal)
        {
            int size = (int) (getAppearance().getContentWidth() * buttonSize);
            if (size < 15)
            {
                size = 15;
            }
        ...
        {
            int size = (int) (getAppearance().getContentHeight() * buttonSize);
            if (size < 15)
            {
                size = 15;
            }
    ...
    /**
     * Add a {@link ISliderMovedListener} to the widget. The listener can be
     * added only once.
     * 
     * @param l
     *            Listener
    ...
    {
        for (final ISliderMovedListener l : sliderMovedHook)
        {
    ...
    public Dimension getMinContentSize()
    {
        ...
        if (horizontal)
        {
            return new Dimension(sliderButton.getMinWidth() + 1,
                    sliderButton.getMinHeight());
        }
        else
        {
            return new Dimension(sliderButton.getMinWidth(),
                    sliderButton.getMinHeight() + 1);
        }
    }
    ...
    public void paintContent(Graphics g, IOpenGL gl)
    {
        ...
        {
            final Button sliderButton = getSliderButton();
    ...
    public Slider clone()
    {
        ...
        result.sliderButton = sliderButton.clone();
