...
public class LWJGLOpenGL implements IOpenGL
{
   ...
   */
    ...
    public LWJGLOpenGL(Thread openGLThread)
    {
    ...
    public boolean[] getBoolean(Attribute attrib)
    {
        ...
        final boolean[] result = new boolean[buf.capacity()];
    ...
    {
        final int pname = getAttrib(attrib);
        final DoubleBuffer buf = BufferUtils.createDoubleBuffer(16);
        GL11.glGetDouble(pname, buf);
        
        final double[] result = new double[buf.capacity()];
        for (int i = 0; i < result.length; i++)
        {
            result[i] = buf.get();
        }
    ...
    public float[] getFloat(Attribute attrib)
    {
        ...
        final float[] result = new float[buf.capacity()];
        for (int i = 0; i < result.length; i++)
        {
            result[i] = buf.get();
        }
    ...
    public int[] getInt(Attribute attrib)
    {
        ...
        final int[] result = new int[buf.capacity()];
        for (int i = 0; i < result.length; i++)
        {
            result[i] = buf.get();
        }
    ...
    public void setOpenGLThread(Thread openGLThread)
    {
        this.openGLThread = openGLThread;
    }
}
