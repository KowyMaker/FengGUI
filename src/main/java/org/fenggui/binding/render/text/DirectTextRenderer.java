...
/**
 * Renders lines of text by drawing each character directly as a single quad.
 * Use this TextRenderer if you update your label (or text) frequently.
 * 
...
public class DirectTextRenderer implements ITextRenderer
{
    ...
    public DirectTextRenderer(InputOnlyStream stream) throws IOException,
    ...
    public void process(InputOutputStream stream) throws IOException,
            IXMLStreamableException
    {
        setFont(stream.processChild("Font", font, ImageFont.getDefaultFont(),
                ImageFont.class));
    }
    ...
    public void render(int x, int y, String text, Color color, Graphics g)
    {
        ...
        final IOpenGL gl = g.getOpenGL();
        int localX = x + g.getTranslation().getX();
        final int localY = y + g.getTranslation().getY() - getLineHeight();
