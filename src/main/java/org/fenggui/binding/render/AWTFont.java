...
public class AWTFont extends org.fenggui.binding.render.Font implements IFont,
        IAWTFont
{
    private Font        font;
    ...
    public AWTFont(Font font, boolean antialiased,
            boolean useFractionalMetrics, boolean underlined)
    {
        this.font = font;
     ...
     * @throws IOException
     */
    public AWTFont(InputOnlyStream stream) throws IOException,
    ...
    private FontMetrics getMetric()
    {
        if (metric == null)
        {
            metric = Toolkit.getDefaultToolkit().getFontMetrics(font);
        }
    ...
    public void process(InputOutputStream stream) throws IOException,
            IXMLStreamableException
    {
        final String name = stream.processAttribute("fontName", "not-set");
        final String typeStr = stream.processAttribute("type", "not-set",
                "plain");
        ...
        useFractionalMetrics = stream.processAttribute("fractionalMetrics",
                true, false);
    }
