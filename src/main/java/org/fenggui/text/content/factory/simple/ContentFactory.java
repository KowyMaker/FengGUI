...
/**
 * This factory uses one style and creates the content using that. The content
 * here is plain text, no special characters are available for formating. It is
 * a very simple and straight forward implementation of a ContentFactory.
 * 
...
public class ContentFactory implements IContentFactory, IXMLStreamable
{
    ...
    public ContentFactory(InputOnlyStream stream) throws IOException,
            ...
            TextAppearance appearance)
    {
        if (content instanceof String)
        {
            final List<AbstractContentPart> result = new ArrayList<AbstractContentPart>(
                    1);
            result.add(new TextPart((String) content, defaultStyleKey,
                    appearance));
            return result;
        }
        else
        {
            throw new IllegalArgumentException(
                    "This ContentFactory only supports input of type String.");
        }
    }
