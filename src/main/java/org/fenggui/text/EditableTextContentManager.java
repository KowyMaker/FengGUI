 ...
 * @version $Revision$
 */
public class EditableTextContentManager implements IEditableTextContentManager,
        IXMLStreamable
{
    ...
    public EditableTextContentManager(EditableTextContentManager data)
    {
        width = data.width;
        height = data.height;
        manager = new ContentManager(data.manager);
        cursor = data.cursor;
    ...
    {
        // this.setMultiline(stream.processAttribute("multiline",
        // this.isMultiline(), false));
        // this.setWordWarping(stream.processAttribute("wordwarped",
        // this.isWordWarping(), false));
        
        cursor = stream.processChild("Cursor", cursor, null, Pixmap.class);
        cursorColor = stream.processChild("CursorColor", cursorColor,
            ...
            TextAppearance appearance)
    {
        ...
        final long pos = manager.findAtomOnPosition(x, this.getSize()
                .getHeight() - y, appearance);
        ...
        switch (key)
        {
            case BACKSPACE:
                if (!isReadonly())
                {
                    ...
                    {
                        final int atom = manager.getSelectionStart();
            ...
            case UP:
                if (!isReadonly() && modifiers.contains(Key.SHIFT))
                {
                    ...
                    final long atom = manager.getActiveAtom();
                    if (atom == -1)
                    {
                        break;
                    }
     ...
     * (non-Javadoc)
     * 
     * @see org.fenggui.text.IComplexTextRendererData#render(int, int,
