...
/**
 * Container that separates two elements either vertically or horizontically.
 * The size of the elements can be adjusted by moving the separator.
 * 
 * 
...
public class SplitContainer extends StandardWidget implements IContainer
{
    ...
    private final DecoratorLayer      barDecorator = new DecoratorLayer();
     ...
     * 
     * @param horizontal
     *            true if split horizontally (i.e. slider is movable up and
     *            downwards), false otherwise
     */
    ...
    private int keepSliderInRange(int newValue)
    {
        ...
        if (horizontal)
        {
            final int firstMinHeight = firstWidget != null ? firstWidget
                    .getMinSize().getHeight() : 0;
            final int secondMinHeight = secondWidget != null ? secondWidget
                    .getMinSize().getHeight() : 0;
        ...
        {
            final int firstMinWidth = firstWidget != null ? firstWidget
                    .getMinSize().getWidth() : 0;
            final int secondMinWidth = secondWidget != null ? secondWidget
                    .getMinSize().getWidth() : 0;
    ...
    public IWidget getWidget(int x, int y)
    {
        ...
        if (firstWidget != null
                && firstWidget.getSize().contains(x - firstWidget.getX(),
                        y - firstWidget.getY()))
        {
            return firstWidget.getWidget(x - firstWidget.getX(), y
                    - firstWidget.getY());
        }
        
        if (secondWidget != null
                && secondWidget.getSize().contains(x - secondWidget.getX(),
                        y - secondWidget.getY()))
        {
            return secondWidget.getWidget(x - secondWidget.getX(), y
                    - secondWidget.getY());
        }
    ...
    public void layout()
    {
        ...
        if (horizontal)
        {
            ...
            if (secondWidget != null)
            {
                ...
                secondWidget.setSize(new Dimension(contentWidth, contentHeight
                        - value - barSize));
            }
        }
        else
        // vertical slider
        {
            ...
            if (secondWidget != null)
            {
                ...
                secondWidget.setY(0);
                secondWidget.setSize(new Dimension(contentWidth - value
                        - barSize, contentHeight));
            }
        }
            ...
            IXMLStreamableException
    {
        ...
        if (getFirstWidget() instanceof StandardWidget
                || getFirstWidget() == null)
        {
            if (stream.startSubcontext("firstWidget"))
            {
                firstWidget = stream.processChild((StandardWidget) firstWidget,
                        XMLTheme.TYPE_REGISTRY);
                stream.endSubcontext();
            }
        }
        
        if (getSecondWidget() instanceof StandardWidget
                || getSecondWidget() == null)
        {
            if (stream.startSubcontext("secondWidget"))
            {
                secondWidget = stream.processChild(
                        (StandardWidget) secondWidget, XMLTheme.TYPE_REGISTRY);
                stream.endSubcontext();
            }
        }
        ...
        if (isHorizontal())
        {
            setPixmap(stream.processChild("HorizontalPixmap", getPixmap(),
                    null, Pixmap.class));
        }
        else
        {
            setPixmap(stream.processChild("VerticalPixmap", getPixmap(), null,
                    Pixmap.class));
        }
    ...
    public Dimension getMinContentSize()
    {
        ...
        final IWidget secondWidget = getSecondWidget();
        ...
        if (isHorizontal())
        {
            return new Dimension(Math.max(firstMinWidth, secondMinWidth),
                    firstMinHeight + getBarSize() + secondMinHeight);
        }
        else
        {
            return new Dimension(firstMinWidth + getBarSize() + secondMinWidth,
    ...
    public void paintContent(Graphics g, IOpenGL gl)
    {
        ...
        final Pixmap pixmap = getPixmap();
        if (pixmap != null)
        {
            g.drawImage(pixmap, x + width / 2 - pixmap.getWidth() / 2, y
                    + height / 2 - pixmap.getHeight() / 2);
        }
    }
