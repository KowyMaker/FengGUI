 ...
 * itself. The overlapping parts of the bigger child widget are clipped. This
 * results in a rectangular window (the view port) in which a part of the child
 * widget is visible. This window can be moved on the child component by calling
 * <code>scrollHorizontal()</code> and <code>scrollVertical()</code> or moving
 * the adjacent scroll bars.<br/>
 * <br/>
 * In case the inner widget is too big to be displayed at once, scroll bars are
 * automatically added to the ScrollContainer and can not be suppressed at the
 * moment. If the internal widget fits into the available space, the scroll bars
 * are not visible. The scroll bar addition and removal is done during the
 * layouting process.<br />
 * <br />
 * Therefore, if the inner widget is modified (e.g. it is a table to which rows
 * are added), the scroll container has to be notified by calling
 * <code>layout()</code>.
 * 
 * @author Johannes, last edited by $Author: marcmenghin $, $Date: 2008-03-06
 *         11:32:27 +0100 (Do, 06 Mrz 2008) $
 * @version $Revision: 633 $
 * @dedication Monkey Island 3 Theme - Using the Row Boat
 *             (http://www.scummbar.com
 *             /mi2/MI3-CD1/35%20-%20Using%20the%20Row%20Boat.mp3)
 */
public class ScrollContainer extends StandardWidget implements IContainer,
        Cloneable
{
    /**
     * The boolean flag indicates if the vertical bar should be placed on the
     * left side of the inner widget.
     */
    private final boolean              verticalBarOnLeft;
    ...
    public ScrollContainer(ScrollContainer widget)
    {
        ...
        verticalBarOnLeft = widget.verticalBarOnLeft;
        horizontalScrollBar = new ScrollBar(widget.horizontalScrollBar);
        verticalScrollBar = new ScrollBar(widget.verticalScrollBar);
        initializeVerticalScrollBar();
        initializeHorizontalScrollBar();
        appearance = new DefaultAppearance(this, widget.appearance);
        buildEvents();
        setMinSize(20, 20);
        updateMinSize();
    }
    ...
    /**
     * Instantiates the horizontal scroll bar and registers its slider
     * listeneres.
     */
    ...
    private void buildEvents()
    {
        if (horizontalScrollBar != null)
        {
            horizontalScrollBar.getSlider().addSliderMovedListener(
                    new ISliderMovedListener() {
                        public void sliderMoved(
                                SliderMovedEvent sliderMovedEvent)
                        {
                            if (innerWidget != null)
                            {
                                placeInnerWidgetHorizontally(sliderMovedEvent
                                        .getPosition());
                            }
                        }
                    });
        ...
        {
            verticalScrollBar.getSlider().addSliderMovedListener(
                    new ISliderMovedListener() {
                        public void sliderMoved(
                                SliderMovedEvent sliderMovedEvent)
                        {
                            if (innerWidget != null)
                            {
                                placeInnerWidgetVertically(sliderMovedEvent
                                        .getPosition());
                            }
                        }
                    });
            ...
            IXMLStreamableException
    {
        ...
        if (getInnerWidget() instanceof StandardWidget
                || getInnerWidget() == null)
        {
            if (stream.startSubcontext("innerWidget"))
            {
                setInnerWidget(stream.processChild(
                        (StandardWidget) innerWidget, XMLTheme.TYPE_REGISTRY));
    ...
    {
        innerWidget
                .setY(-(int) (sliderValue * getVerticalScrollSpace())
                        + (displayHorizontalScrollBar && showScrollbars ? horizontalScrollBar
                                .getHeight() : 0));
    }
    ...
    /**
     * Returns the Widget below a specified point. This is either the inner
    ...
    public IWidget getWidget(int x, int y)
    {
        ...
        if (showScrollbars
                ...
                && horizontalScrollBar.getSize().contains(
                        x - horizontalScrollBar.getX(),
                        y - horizontalScrollBar.getY()))
        {
            return horizontalScrollBar.getWidget(
                    x - horizontalScrollBar.getX(),
                    y - horizontalScrollBar.getY());
        }
        if (showScrollbars
                && displayVerticalScrollBar
                && verticalScrollBar.getSize().contains(
                        x - verticalScrollBar.getX(),
        ...
        if (innerWidget != null)
        {
            ...
            {
                // modX = (int) ((innerWidget.getSize().getWidth() -
                // getAppearance().getContentWidth() +
                // ((displayVerticalScrollBar)?(verticalBarOnLeft?verticalScrollBar.getSize().getWidth():0):0))
                // * (-getHorizontalScrollQuotient()) + x -
                // ((displayVerticalScrollBar)?(verticalBarOnLeft?verticalScrollBar.getSize().getWidth():0):0));
                modX = (int) ((innerWidget.getSize().getWidth()
                        - getAppearance().getContentWidth() + (displayVerticalScrollBar ? verticalScrollBar
                        .getWidth() : 0))
                        * -getHorizontalScrollQuotient() + x);
            }
            ...
            {
                modY = (int) ((innerWidget.getSize().getHeight()
                        - getAppearance().getContentHeight() + (displayHorizontalScrollBar ? horizontalScrollBar
                        .getSize().getHeight() : 0))
                        * getVerticalScrollQuotient() + y - (displayHorizontalScrollBar ? horizontalScrollBar
                        .getSize().getHeight() : 0));
            }
            else
            {
                modY += innerWidget.getSize().getHeight()
     ...
     * 
     * @return computes
     *         <code>innerWidget.getX() / getHorizontalScrollSpace()</code>
     */
    public double getHorizontalScrollQuotient()
    {
        return innerWidget.getX() / getHorizontalScrollSpace();
    }
    
    /**
     * Returns how far the inner widget is away from the bottom. XXX:
     * getHorizontalScrollQuotient() works considerably different! Make both
     * methods do the same on their respective dimension.
     */
    public double getVerticalScrollQuotient()
    {
        if (displayHorizontalScrollBar)
        {
            return (horizontalScrollBar.getHeight() - innerWidget.getY())
                    / getVerticalScrollSpace();
        }
        else
        {
            return -innerWidget.getY() / getVerticalScrollSpace();
        }
    }
    ...
    public void scrollHorizontal(double percent)
    {
        ...
        final double d = percent * getHorizontalScrollSpace();
    ...
    public void scrollVertical(double percent)
    {
        ...
        final double d = percent * getVerticalScrollSpace();
    ...
    public void stepScrollHorizontal(boolean right)
    {
        ...
        final double horScrollSpace = getHorizontalScrollSpace();
        
        final double step = 1.0 / (horScrollSpace / getWidth());
    ...
    public void stepScrollVertical(boolean up)
    {
        ...
        final double verScrollSpace = getVerticalScrollSpace();
        
        final double step = 1.0 / (verScrollSpace / getHeight());
    ...
    public void setInnerWidget(IWidget innerWidget)
    {
        ...
        // hook events
        innerWidget.addMinSizeChangedListener(sizeChangedListener);
    ...
    /**
     * Layouts the ScrollContainer. That is the inner widget and both scroll
     * bars (if required).
     */
    @Override
    public void layout()
    {
        ...
        /*
         * The basic strategy goes like this: first we check whether we need the
         * scroll bars to make the view port movable on the inner widget (step
         * 1). Then we know whether we the vertical and the horizontal scroll
         * bar. Adding a scroll bar reduces the size of the view port. We thus
         * need to check whether we need to enable a scroll bar that has been
         * unnecessary before concidering the activation of the other scroll bar
         * (step 2). It is now save to setup the inner widget (step 3). Finally,
         * we setup the scroll bars in step 4.
         */
        ...
        /*
         * the view port is the space between the scroll bars (if present) in
         * which the inner widget is displayed. By adding scroll bars, the size
         * of the view port is reduced (obviously).
         */
        ...
        final int innerWidgetMinWidth = getInnerWidget().getMinSize()
                .getWidth();
        final int innerWidgetMinHeight = getInnerWidget().getMinSize()
                .getHeight();
        int innerWidgetWidth = getInnerWidget().getSize().getWidth();
        int innerWidgetHeight = getInnerWidget().getSize().getHeight();
        
        /*
         * STEP 1: We need to check whether we need to add or remove scroll bars
         * in order to enable the user to move the inner widget in the
         * ScrollContainer.
         */
        ...
        /*
         * STEP 2: by adding the vertical scroll bar, the size of the view port
         * is reduced, so that it may by that we also need to horizontal scroll
         * bar after all.
         */
        if (displayVerticalScrollBar && !displayHorizontalScrollBar
                && viewPortWidth < innerWidgetMinWidth)
        {
            displayHorizontalScrollBar = true;
            if (showScrollbars)
            {
        ...
        if (!displayVerticalScrollBar && displayHorizontalScrollBar
                && viewPortHeight < innerWidgetMinHeight)
        {
            displayVerticalScrollBar = true;
            if (showScrollbars)
            {
        ...
        /*
         * STEP 3: ok, now we know whether we need scroll bars to completely
         * display the inner widget. We set the size of the inner widget such
         * that it receives the min. with in the dimension where it is bigger
         * than the view port. In case a dimension is smaller then the available
        ...
        /*
         * STEP 4: adjust slider height according to the new size of the inner
         * widget... if inner Widget wider than ScrollContainer
         */
        if (displayHorizontalScrollBar)
        {
            horizontalScrollBar.setSize(viewPortWidth,
                    horizontalScrollBar.getMinHeight());
            horizontalScrollBar.setXY(0, 0);
            ...
            d = 10.0 / (innerWidgetMinWidth - viewPortWidth);
        ...
        if (displayVerticalScrollBar)
        {
            verticalScrollBar.setSize(verticalScrollBar.getMinWidth(),
                    viewPortHeight);
            if (verticalBarOnLeft)
            {
            ...
            {
                verticalScrollBar.setXY(viewPortWidth, contentHeight
                        - viewPortHeight);
            }
            ...
            d = 10.0 / (innerWidgetMinHeight - viewPortHeight);
    ...
    public void addWidget(IWidget... w)
    {
        ...
        else
        {
            throw new IllegalArgumentException(
                    "At least one widget needs to be added.");
        }
    ...
    {
        final IWidget w = getNextWidget(start);
    ...
    {
        final IWidget w = getPreviousWidget(start);
    ...
    /**
     * @return True if the vertical bar of this scroll container is placed on
     *         the left side of the inner widget. False otherwise.
     */
    ...
    public void paintContent(Graphics g, IOpenGL gl)
    {
        ...
        final ScrollBar horizontalScrollBar = this.getHorizontalScrollBar();
        if (showScrollbars && displayHorizontalScrollBar)
        {
            g.translate(horizontalScrollBar.getX(), horizontalScrollBar.getY());
            horizontalScrollBar.paint(g);
            g.translate(-horizontalScrollBar.getX(),
                    -horizontalScrollBar.getY());
        }
        
        final ScrollBar verticalScrollBar = this.getVerticalScrollBar();
        if (showScrollbars && displayVerticalScrollBar)
        {
        ...
        final int verticalSBMinWidth = !(displayVerticalScrollBar && showScrollbars) ? 0
                : verticalScrollBar.getMinWidth();
        final int horizontalSBMinHeight = !(displayHorizontalScrollBar && showScrollbars) ? 0
                : horizontalScrollBar.getMinHeight();
        
        // System.out.println("clip of "+ this.getId() +" before :" +
        // g.getClipSpace());
        
        if (this.isVerticalBarOnLeft())
        {
            // g.setClipSpace(verticalSBMinWidth, horizontalSBMinHeight,
            // appearance.getContentWidth() - verticalSBMinWidth,
            // appearance.getContentHeight() - horizontalSBMinHeight);
            g.addClipSpace(verticalSBMinWidth, horizontalSBMinHeight,
                    appearance.getContentWidth() - verticalSBMinWidth,
                    appearance.getContentHeight() - horizontalSBMinHeight);
        }
        else
        {
            g.addClipSpace(0, horizontalSBMinHeight,
                    appearance.getContentWidth() - verticalSBMinWidth,
                    appearance.getContentHeight() - horizontalSBMinHeight);
        }
        
        // System.out.println("clip of "+ this.getId() +"is now :" +
        // g.getClipSpace());
    ...
    public void removeInnerWidget()
    {
        if (innerWidget != null)
        {
            innerWidget.removeMinSizeChangedListener(sizeChangedListener);
            innerWidget.removedFromWidgetTree();
            innerWidget.setParent(null);
            if (getDisplay() != null)
            {
    ...
    {
        final List<IWidget> result = new ArrayList<IWidget>(1);
    ...
    public ScrollContainer clone()
    {
        ...
        result.appearance = new DefaultAppearance(this, appearance);
        
        result.horizontalScrollBar = horizontalScrollBar.clone();
        result.verticalScrollBar = verticalScrollBar.clone();
