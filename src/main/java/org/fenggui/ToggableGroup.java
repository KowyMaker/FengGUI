...
/**
 * Manages toggable items. Instances of this class are usually used for radio
 * buttons, within lists and tables and shall be seen as read only objects,
 * because the widgets in question handle the <code>ToggableWidgetGroup</code>
 * themself.<br/>
 * <br/>
 * The number of selectable items can be specified either trough the constructor
 * or <code>setNumberOfSelectableItems</code>. For single selection, the correct
 * value is <code>SINGLE_SELECTION</code>, while for arbitrary selection the
 * value is <code>MULTIPLE_SELECTION</code>. To allow not more than three items
 * to be selected at a time the corresponding value is 3.
 * 
...
public class ToggableGroup<E extends IToggable<?>>
{
    ...
    private final java.util.List<E>                         selected                = new ArrayList<E>();
    private final java.util.List<ISelectionChangedListener> selectionChangedHook    = new ArrayList<ISelectionChangedListener>();
    ...
    /**
     * Creates a new <code>ToggableWidgetGroup</code> instance. This group
    ...
    {
        final java.util.List<E> tmpSel = new ArrayList<E>(selected);
        selected.clear();
        
        for (final E item : tmpSel)
        {
    ...
    public void setSelected(IWidget source, E w, boolean b)
    {
        if (b)
        {
            ...
            if (numberOfSelectableItems == SINGLE_SELECTION
                    || numberOfSelectableItems >= 1)
            {
                // remove items that are too much after the selection change
                if (selected.size() > 0)
                {
                    for (int i = selected.size() - 1; i >= numberOfSelectableItems
                            && i >= 0; i--)
                    {
                        final E s = selected.get(i);
                        s.setSelected(false);
                        selected.remove(i);
    ...
    /**
     * Returns the first currently selected item. Makes probably only sense for
     * single selection.
     * 
    ...
    /**
     * Returns an array of the values of all currently selected items. It
     * requires the class of the values to instantiate the array.
     * 
     * @param returnType
    ...
    {
        final E[] array = (E[]) Array.newInstance(returnType, selected.size());
    ...
    {
        final SelectionChangedEvent e = new SelectionChangedEvent(source, t, s);
        
        for (final ISelectionChangedListener l : selectionChangedHook)
        {
