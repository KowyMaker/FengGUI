...
/**
 * A Menu Bar is the horizontal thing right below the Window header or the top
 ...
 *         2009-03-15 16:16:34 +0100 (So, 15 Mär 2009) $
...
public class MenuBar extends StandardWidget implements IMenuChainElement
{
    ...
    private final LinkedList<MenuItem> items         = new LinkedList<MenuItem>();
    ...
    public void closeBackward()
    {
         ...
         * The menu would reopen if the menu bar gets here the keyfocus. It
         * makes on the otherhand no sence that the menu keeps it's key focus.
         * Thats why we give no widget the key focus when the user close the
         * menu.
         */
        if (getDisplay() != null)
        {
    ...
    public void mouseMoved(int displayX, int displayY)
    {
        ...
        for (final MenuItem item : items)
        {
            final int itemWidth = item.getPreferredSize().getWidth() + GAP;
            
            if (mouseX >= x && mouseX - x < itemWidth)
            {
                mouseOver = item;
                
                if (!mouseOver.getMenu().equals(currentlyOpen)
                        && currentlyOpen != null)
                {
                    currentlyOpen.closeForward();
    ...
    public Dimension getMinContentSize()
    {
        ...
        for (final MenuItem item : this.getMenuBarItems())
        {
            itemWidth += item.getPreferredSize().getWidth();
            itemHeight = Math.max(item.getPreferredSize().getHeight(),
                    itemHeight);
        }
    ...
    public void paintContent(Graphics g, IOpenGL gl)
    {
        ...
        for (final MenuItem item : menuBar.getMenuBarItems())
        {
            
            final int itemWidth = item.getPreferredSize().getWidth();
            
            if (item.equals(aktiveMenu))
            {
                appearance.getSelectionUnderlay().paint(g, x, 0,
                        itemWidth + GAP, appearance.getContentHeight());
            }
            
            if (item.equals(menuBar.getMouseOver()))
            {
                appearance.getHoverUnderlay().paint(g, x, 0, itemWidth + GAP,
                        appearance.getContentHeight());
            }
            
            item.render(x + GAP / 2, 0, itemWidth,
                    appearance.getContentHeight(), aktiveMenu, appearance, g);
