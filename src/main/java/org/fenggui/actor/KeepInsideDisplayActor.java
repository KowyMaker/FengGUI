...
/**
 * Repositions windows which are outside the display to the nearest border of
 * the display. A window widget can be moved half outside the display except the
 * top as it can only be moved there.
 * 
        ...
        listener = new IDisplayResizedListener() {
            ...
            public void displayResized(DisplayResizedEvent displayResizedEvent)
            {
                for (final IWidget widget : display.getWidgets())
                {
                    ...
                    {
                        final Dimension size = widget.getSize();
                        ...
                        if (x + 3 * size.getWidth() / 4 > display.getSize()
                                .getWidth())
                        {
                            widget.setX(display.getSize().getWidth() - 3
                                    * size.getWidth() / 4);
                        }
                        ...
                        if (y + size.getHeight() > display.getSize()
                                .getHeight())
                        {
                            widget.setY(display.getSize().getHeight()
                                    - size.getHeight());
                        }
                    }
                }
