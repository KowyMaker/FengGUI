 ...
 * yourDisplay.display() method call so the actor is able to save a complete
 * screenshot.
 * 
...
public class ScreenshotActor implements IActor
{
    ...
    private final IEventListener keylistener       = new IEventListener() {
                                                               ...
                                                               Event event)
                                                       {
                                                           ...
                                                           {
                                                               if (((KeyReleasedEvent) event)
                                                                       .getKeyClass() == Key.F12)
                                                               {
                                                                   takeScreenshot(new File(
   ...
   */
    ...
    /**
     * This method should be called after the Display's display() Method. This
    ...
    /**
     * Takes a screenshot of the current frame. This method is entirely copied
     * from http://www.javagaming.org/forums/index.php?topic=8747.0
     * 
     * @param gl
    ...
    private static void screenshot(IOpenGL gl, int width, int height, File file)
    {
        try
        {
            ...
            final int fileLength = TARGA_HEADER_SIZE + width * height * 3;
            out.setLength(fileLength);
            final MappedByteBuffer image = ch.map(
                    FileChannel.MapMode.READ_WRITE, 0, fileLength);
            ...
            final ByteBuffer bgr = image.slice();
