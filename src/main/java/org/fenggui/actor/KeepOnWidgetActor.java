...
public class KeepOnWidgetActor implements IActor
{
   ...
   */
        ...
        if (this.widget != null)
        {
            ...
            else
            {
                throw new IllegalArgumentException(
                        "Different widget then used to hook this actor");
            }
        }
    }
    ...
    private void positionWidget(IWidget widget)
    {
        if (widget != null)
        {
            final Point pos = align.alignBox(new Point(target.getDisplayX(),
                    target.getDisplayY()), target.getSize(), widget.getSize());
