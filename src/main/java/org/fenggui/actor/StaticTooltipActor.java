...
/**
 * Positions a tooltip at a position relative to the widget. The tooltip data is
 * received from the widgets custom data storage. It will only be displayed on
 * widgets that have this data. The tooltip will be displayed as soon as the
 * widget gets the focus. When the focus is lost the tooltip will be removed
 * again.<br />
 * <br />
...
public abstract class StaticTooltipActor implements IActor
{
    ...
    private final ISizeChangedListener     sizeListener;
    private final IPositionChangedListener positionListener;
    ...
    public StaticTooltipActor()
    {
        sizeListener = new ISizeChangedListener() {
            ...
            public void sizeChanged(SizeChangedEvent event)
            {
                if (isTooltipReady())
                {
                    positionTooltipOnWidget(lastWidget, tooltip,
                            display.getSize());
                }
            }
        };
        ...
        if (listener == null)
        {
            listener = new IEventListener() {
                ...
                public void processEvent(Event event)
                {
                    ...
                    {
                        final FocusEvent fevent = (FocusEvent) event;
                        
                        if (fevent.isFocusLost())
                        {
                            hideTooltip();
                            lastWidget
                                    .removePositionChangedListener(positionListener);
                        ...
                        if (fevent.isFocusGained())
                        {
                            ...
                            lastWidget
                                    .addPositionChangedListener(positionListener);
     ...
     * @param space
     */
    protected abstract void positionTooltipOnWidget(IWidget widget,
            IWidget tooltip, Dimension space);
    
    private void hideTooltip()
    {
        if (tooltip != null && tooltip.isInWidgetTree())
        {
            final IWidget parent = tooltip.getParent();
    ...
    private void showTooltip(IWidget widget)
    {
        ...
        final Object data = getTooltipDataFromWidget(widget);
        setTooltip(data);
        if (data != null)
        {
    ...
    private void displayTooltip(IWidget widget)
    {
        ...
        if (display.getPopupWidget() != null
                && display.getPopupWidget().isInWidgetTree())
        {
    ...
    /**
     * Should be used to add the tooltip data to the widget. This uses the
     * custom data storage (with HashMap) from the widget.
     * 
     * @see Util
     * @param widget
     *            widget to add the data.
     * @param data
     *            data used by the tooltip.
     * @return the previous tooltip data associated with the widget, or null if
     *         there was no data. (A null return can also indicate that the
     *         tooltip data previously associated with the widget was null.)
     */
