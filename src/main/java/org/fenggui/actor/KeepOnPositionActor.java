...
public class KeepOnPositionActor implements IActor
{
   ...
   */
    ...
    public void hook(IWidget widget)
    {
        if (this.widget != null)
        {
            throw new IllegalStateException(
                    "Actor already hooked to an other widget. Unhook that first.");
        }
        
        widget.getDisplay().getBinding().addDisplayResizedListener(listener);
        this.widget = widget;
        // initial positioning
        position(widget.getDisplay().getWidth(), widget.getDisplay()
                .getHeight());
    }
    ...
    public void unHook(IWidget widget)
    {
        ...
        {
            throw new IllegalStateException(
                    "Actor already hooked on an other widget. Unhook that first.");
        }
        
        this.widget.getDisplay().getBinding()
                .removeDisplayResizedListener(listener);
        this.widget = null;
    }
    ...
    private void position(int width, int height)
    {
        ...
        final Point newPosition = align.alignBox(new Point(0, 0), size,
                widget.getSize());
