...
/**
 * Implementation of a statefull Widget. All widgets where the user is able to
 * interact with should be derived from this abstract class.
 * 
 * This widget has the following states: default, focused, hovered, error and
 * disabled.
 * 
 * @author marcmenghin, last edited by $Author$, $Date$
 * @version $Revision$
 */
public abstract class StatefullWidget<T extends DecoratorAppearance> extends
        ObservableWidget
{
    ...
    public void mouseEntered(MouseEnteredEvent mouseEnteredEvent)
    {
        ...
        Binding.getInstance().getCursorFactory()
                .getCursor(defaultHoverCursorType).show();
    ...
    public void mouseExited(MouseExitedEvent mouseExitedEvent)
    {
        ...
        Binding.getInstance().getCursorFactory().getCursor(CursorType.DEFAULT)
                .show();
    ...
    public boolean isFocused()
    {
        if (this.getDisplay() != null
                && this.getDisplay().getFocusedWidget() == this)
        {
            return true;
        }
        else
