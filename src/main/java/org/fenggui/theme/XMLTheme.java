 ...
 * 
 * Please note that you can switch to loading via the class loader by enabling
 * Binding.setUseClassLoader(true).
 * 
 * @author Johannes Schaback, last edited by $Author:Schabby $, $Date:2007-08-11
 *         12:11:38 +0200 (Sat, 11 Aug 2007) $
...
public class XMLTheme implements ITheme
{
    ...
    static
    {
        ...
        TYPE_REGISTRY.register("ComplexTextRendererData",
                TextContentManager.class);
        TYPE_REGISTRY.register("AdvancedTextRendererData",
                EditableTextContentManager.class);
        ...
        COMPLEXTEXTRENDERERDATA_REGISTRY.register("ComplexTextRendererData",
                TextContentManager.class);
        COMPLEXTEXTRENDERERDATA_REGISTRY.register("AdvancedTextRendererData",
                EditableTextContentManager.class);
        ...
        TOOLTIPMANAGER_REGISTRY.register("TooltipManager",
                LabelTooltipManager.class);
    }
    
    public XMLTheme(String xmlThemeFile) throws IOException,
            IXMLStreamableException
    {
        /* extract path to xml file */
        if (xmlThemeFile.indexOf('/') != -1)
        {
            resourcePath = xmlThemeFile.substring(0,
                    xmlThemeFile.lastIndexOf('/') + 1);
        }
        else
        {
            resourcePath = xmlThemeFile.substring(0,
        ...
        final Element el = document.getChild("FengGUI:init");
        ...
        if (el != null)
        {
            ...
            final List<IXMLStreamable> contents = new ArrayList<IXMLStreamable>();
    ...
    public void setUp(IWidget widget)
    {
        if (!(widget instanceof StandardWidget))
        {
            throw new IllegalArgumentException("widget "
                    ...
                    + " is not a StandardWidget!");
        }
        
        final StandardWidget w = (StandardWidget) widget;
        
        final String type = findSupertype(widget);
        ...
        final Element el = document.getChild(type);
        ...
        catch (final IXMLStreamableException e)
        {
            throw new RuntimeException(e);
        }
    ...
    {
        final String warningsStr = stream.getWarningsAsString().trim();
