...
/**
 * An exception thrown when the construction of a child object fails. Primarly
 * used as a wrapper exception for the numerious different exceptions thrown by
 * the reflection API.
 * 
...
public class ChildConstructionException extends IXMLStreamableException
{
    ...
    public static ChildConstructionException createMultipleDefinitionsException(
            String name, IParseContext parsingContext)
    {
        return new ChildConstructionException(
                "multiple definitions for the element " + name + "\n\n",
                parsingContext);
    }
}
