 ...
 * items used in this list are <code>Items</code> rather than widgets.
 * 
 * 
 * 
 * @author Johannes, last edited by $Author: marcmenghin $, $Date: 2009-04-22
 *         22:34:09 +0200 (Mi, 22 Apr 2009) $
...
public class List extends ModelWidget<EntryAppearance, IListModel>
{
     ...
     * 
     * @param b
     *            Set to true for the Item to toggle between selected and
     *            deselected as you click it. false to always set as true,
     *            regardless of how many times in a row it is clicked.
     * 
     */
    ...
    {
        final java.util.List<Item> items = new ArrayList<Item>(texts.length);
        for (final String itemText : texts)
        {
    ...
    public void mouseMoved(int displayX, int displayY)
    {
        ...
        final int frombottom = this.getSize().getHeight()
                - this.getMinSize().getHeight();
        final int currentY = frombottom > 0 ? frombottom : 0;
    ...
    public void paintContent(Graphics g, IOpenGL gl)
    {
        ...
        final EntryAppearance appearance = getAppearance();
        
        int y = appearance.getContentHeight();
        final int x = 0;
        final int width = appearance.getContentWidth();
        ...
        for (final Item item : items)
        {
            ...
            if (g.getClipSpace().getIntersection(
                    new Rectangle(this.getDisplayX() + x, this.getDisplayY()
                            + y, width, size.getHeight())) == null)
            {
                continue;
            }
            
            // draw item
            item.render(x, y, width, size.getHeight(), hoveredItem, appearance,
                    g);
