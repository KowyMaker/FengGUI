 ...
 * 
 * @author Johannes, last edited by $Author: marcmenghin $, $Date: 2009-03-22
 *         16:58:20 +0100 (So, 22 Mär 2009) $
...
public class ProgressBar extends StandardWidget
{
    ...
    public void paintContent(Graphics g, IOpenGL gl)
    {
        if (horizontal)
        {
            appearance.getSelectionUnderlay().paint(g, 0, 0,
                    (int) (appearance.getContentWidth() * this.getValue()),
                    appearance.getContentHeight());
        }
        else
        {
            appearance.getSelectionUnderlay().paint(g, 0, 0,
                    appearance.getContentWidth(),
        ...
        if (!textData.isEmpty())
        {
            ...
            final int x = org.fenggui.util.Alignment.MIDDLE.alignX(
                    appearance.getContentWidth(), size.getWidth());
            final int y = org.fenggui.util.Alignment.MIDDLE.alignY(
                    appearance.getContentHeight(), size.getHeight())
                    + size.getHeight();
    ...
    public void sizeChanged(SizeChangedEvent event)
    {
        ...
        textData.adaptChange(appearance.getContentWidth(),
                appearance.getContentHeight(), appearance);
    }
}
