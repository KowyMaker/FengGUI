...
/**
 * This widget adds various listener methods to all subclasses. It enriches the
 * widgets with common events.
 * 
 * @author Johannes Schaback, last edited by $Author: marcmenghin $, $Date:
 *         2007-11-29 18:32:35 +0100 (Do, 29 Nov 2007) $
...
public abstract class ObservableWidget extends StandardWidget
{
    ...
    /**
     * Returns true if this widget is enabled. If it is enabled it can recieve
     * events.
     * 
     * @return
     */
    ...
    public void activated(ActivationEvent e)
    {
        ...
        for (final IActivationListener l : activationHook)
        {
            l.widgetActivationChanged(e);
        }
    }
    
    private ArrayList<IActivationListener>    activationHook    = new ArrayList<IActivationListener>(
                                                                        0);
    private ArrayList<IMouseEnteredListener>  mouseEnteredHook  = new ArrayList<IMouseEnteredListener>(
                                                                        0);
    private ArrayList<IMouseMovedListener>    mouseMovedHook    = new ArrayList<IMouseMovedListener>(
                                                                        0);
    private ArrayList<IMouseExitedListener>   mouseExitedHook   = new ArrayList<IMouseExitedListener>(
                                                                        0);
    private ArrayList<IMousePressedListener>  mousePressedHook  = new ArrayList<IMousePressedListener>(
                                                                        0);
    private ArrayList<IMouseReleasedListener> mouseReleasedHook = new ArrayList<IMouseReleasedListener>(
                                                                        0);
    private ArrayList<IFocusListener>         focusGainedHook   = new ArrayList<IFocusListener>(
                                                                        0);
    private ArrayList<IMouseDraggedListener>  mouseDraggedHook  = new ArrayList<IMouseDraggedListener>(
                                                                        0);
    private ArrayList<IMouseWheelListener>    mouseWheeledHook  = new ArrayList<IMouseWheelListener>(
                                                                        0);
    private ArrayList<IKeyPressedListener>    keyPressedHook    = new ArrayList<IKeyPressedListener>(
                                                                        0);
    private ArrayList<IKeyReleasedListener>   keyReleasedHook   = new ArrayList<IKeyReleasedListener>(
                                                                        0);
    private ArrayList<IKeyTypedListener>      keyTypedHook      = new ArrayList<IKeyTypedListener>(
                                                                        0);
    private ArrayList<IMouseListener>         mouseHook         = new ArrayList<IMouseListener>(
                                                                        0);
    private ArrayList<IKeyListener>           keyHook           = new ArrayList<IKeyListener>(
                                                                        0);
    ...
    public void mouseEntered(MouseEnteredEvent mouseEnteredEvent)
    {
        ...
        for (final IMouseEnteredListener l : mouseEnteredHook)
        {
            l.mouseEntered(mouseEnteredEvent);
        }
        
        for (final IMouseListener l : mouseHook)
        {
            l.mouseEntered(mouseEnteredEvent);
        }
    }
    ...
    public void mouseExited(MouseExitedEvent mouseExitedEvent)
    {
        ...
        for (final IMouseExitedListener l : mouseExitedHook)
        {
    ...
    public void mousePressed(MousePressedEvent mousePressedEvent)
    {
        ...
        for (final IMousePressedListener l : mousePressedHook)
        {
    ...
    public void mouseMoved(int displayX, int displayY)
    {
        if (mouseHook.isEmpty() && mouseMovedHook.isEmpty() || !enabled
                || !isVisible() || !isInWidgetTree())
        {
            return;
        }
        
        final MouseMovedEvent e = new MouseMovedEvent(null, displayX, displayY,
                getDisplay().getKeyPressTracker().getModifiers());
        
        for (final IMouseMovedListener l : mouseMovedHook)
        {
            l.mouseMoved(e);
        }
    ...
    public void mouseDragged(MouseDraggedEvent mouseDraggedEvent)
    {
        ...
        for (final IMouseDraggedListener l : mouseDraggedHook)
        {
    ...
    public void mouseReleased(MouseReleasedEvent mouseReleasedEvent)
    {
        ...
        for (final IMouseReleasedListener l : mouseReleasedHook)
        {
    ...
    public void keyPressed(KeyPressedEvent keyPressedEvent)
    {
        ...
        for (final IKeyPressedListener l : keyPressedHook)
        {
            l.keyPressed(keyPressedEvent);
        }
        
        for (final IKeyListener l : keyHook)
        {
            l.keyPressed(keyPressedEvent);
        }
    ...
    public void keyReleased(KeyReleasedEvent keyReleasedEvent)
    {
        ...
        for (final IKeyReleasedListener l : keyReleasedHook)
        {
    ...
    public void keyTyped(KeyTypedEvent keyTypedEvent)
    {
        ...
        for (final IKeyTypedListener l : keyTypedHook)
        {
    ...
    public void focusChanged(FocusEvent focusGainedEvent)
    {
        ...
        for (final IFocusListener l : focusGainedHook)
        {
    ...
    public void mouseWheel(MouseWheelEvent e)
    {
        ...
        for (final IMouseWheelListener l : mouseWheeledHook)
        {
            l.mouseWheel(e);
        }
        ...
        if (getDisplay() != null)
        {
            ...
            globalListener = new IEventListener() {
                ...
                public void processEvent(Event event)
                {
                    ...
                    {
                        final MouseReleasedEvent mouseReleasedEvent = (MouseReleasedEvent) event;
                        if (mouseReleasedEvent.getSource() != ObservableWidget.this
                                && mousePressedOnWidget)
                        {
                            ObservableWidget.this
