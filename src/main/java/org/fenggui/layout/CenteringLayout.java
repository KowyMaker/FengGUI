...
/**
 * Takes a single widget and centers it, horizontally and/or vertically, at its
 * current size, within the available space.
 * 
 * @author Sam Bayless
 * 
 */
public class CenteringLayout extends LayoutManager
{
    ...
    /**
     * The amount to shift the center of the widget by. Can be positive or
     * negative. Only applied if centering horizontally (or both).
     */
    private int         xOffset = 0;
    
    /**
     * The amount to shift the center of the widget by. Can be positive or
     * negative. Only applied if centering vertically (or both).
     */
     ...
     * 
     * @param centerStyle
     *            The type of centering to provide - horizontal, vertical, or
     *            both.
     */
    ...
    public void doLayout(Container container, List<IWidget> content)
    {
        ...
        final int left = container.getAppearance().getLeftMargins();
        final int bottom = container.getAppearance().getBottomMargins();
        ...
        for (final IWidget widget : content)
        {
            ...
            if (centerStyle == CenterStyle.BOTH
                    || centerStyle == CenterStyle.HORIZONTAL)
            {
                xPosition = left + contentWidth / 2
                        - widget.getSize().getWidth() / 2 + getXOffset();
            }
            
            if (centerStyle == CenterStyle.BOTH
                    || centerStyle == CenterStyle.VERTICAL)
            {
                yPosition = bottom + contentHeight / 2
                        - widget.getSize().getHeight() / 2 + getYOffset();
            }
     ...
     * 
     * @param offset
     *            the amount to offset the center by. Can be positive or
     *            negative.
     */
