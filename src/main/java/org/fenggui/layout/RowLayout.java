...
/**
 * Layouts Widgets in a vertical or horizontal row. The available inner size of
 * the container is shared equally to the Widgets.
 * 
...
public class RowLayout extends LayoutManager
{
     ...
     * 
     * @param horizontal
     *            specifies whether the row will be a vertical or horizontal in
     *            the container
     */
     ...
     * (non-Javadoc)
     * 
     * @see windowtoolkit.ILayoutManager#doLayout(windowtoolkit.Container,
    ...
    public void doLayout(Container container, List<IWidget> content)
    {
        ...
        final int reqSpace = horizontal ? getSumOfAllWidths(content)
                : getSumOfAllHeights(content);
        
        freeSpacePerComp = horizontal ? container.getAppearance()
                .getContentWidth() : container.getAppearance()
                .getContentHeight();
        ...
        freeSpacePerComp = (freeSpacePerComp - reqSpace) / expandableWidgets;
        ...
        for (int i = 0; i < content.size(); i++)
        {
            ...
            if (horizontal)
            {
                ...
                setValidSize(w, (int) freeSpacePerComp + getValidMinWidth(w),
                        container.getAppearance().getContentHeight());
                
                w.setX(x);
                x += w.getSize().getWidth();
                w.setY(container.getAppearance().getContentHeight() / 2
                        - w.getSize().getHeight() / 2);
            }
            else
            {
                w = content.get(content.size() - i - 1);
                
                setValidSize(w, container.getAppearance().getContentWidth(),
                        (int) freeSpacePerComp + getValidMinHeight(w));
                
                w.setY(y);
                y += w.getSize().getHeight();
                w.setX(container.getAppearance().getContentWidth() / 2
                        - w.getSize().getWidth() / 2);
            }
        }
