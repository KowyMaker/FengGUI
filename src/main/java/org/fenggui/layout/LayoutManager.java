...
/**
 * Layout Managers layout the position and size of Widgets. They are used within
 * Containers to arrange their children Widgets. They only affect the children
...
public abstract class LayoutManager implements IXMLStreamable
{
    ...
    public void setValidHeight(IWidget widget, int height)
    {
        if (widget.getSize().getHeight() > height)
        {
            if (widget.isShrinkable())
            {
                widget.setSize(new Dimension(widget.getSize().getWidth(),
                        height));
            }
        }
        else
    ...
    public void setValidWidth(IWidget widget, int width)
    {
        if (widget.getSize().getWidth() > width)
        {
            if (widget.isShrinkable())
            {
                widget.setSize(new Dimension(width, widget.getSize()
                        .getHeight()));
            }
        }
        else
     ...
     * 
     * @param container
     *            the Container whose children are to be layouted
     * @param content
     *            the list of children Widget
     */
    public abstract void doLayout(Container container,
            java.util.List<IWidget> content);
    
    /**
     * Computes and sets the minimum size of the specified Container.
     * 
     * @param container
     *            the container whose minimum size is to be updated
     * @param content
