...
public class BorderLayout extends LayoutManager
{
    ...
    public void doLayout(Container container, List<IWidget> content)
    {
        ...
        {
            final BorderLayoutData bld = (BorderLayoutData) c.getLayoutData();
            ...
            if (bld == null)
            {
                 ...
                 * System.err.println("The container ("
                 * +container+") has an BorderLayout, but this child ("+c+") " +
        ...
        for (final IWidget w : content)
        {
            ...
            {
                w.setX(widthWest);
                w.setY(container.getAppearance().getContentHeight()
                        - heightNorth);
                setValidSize(w, container.getAppearance().getContentWidth()
                        - widthWest - widthEast, getValidMinHeight(w));
            }
            else if (bld.getValue() == BorderLayoutData.SOUTH_VALUE)
            {
                w.setX(widthWest);
            ...
            {
                w.setX(container.getAppearance().getContentWidth()
                        - getValidMinWidth(w));
                w.setY(0);
                setValidSize(w, getValidMinWidth(w), container.getAppearance()
                        .getContentHeight());
            }
            else if (bld.getValue() == BorderLayoutData.WEST_VALUE)
            {
            ...
            { // CENTER
                ...
                w.setY(heightSouth);
                setValidSize(w, container.getAppearance().getContentWidth()
                        - widthEast - widthWest, container.getAppearance()
                        .getContentHeight() - heightSouth - heightNorth);
            }
        }
